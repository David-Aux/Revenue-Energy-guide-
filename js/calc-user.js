  ...
  function l4u__buildBOM(s,bi){
    ...
    s.bom=bom;
    s.budget=l4u__calcBudgetRange(s,bi);
    s.total=s.budget.min;
    return s;
  }
  ...
  function l4u__renderBOM(s){
    var body=document.getElementById('l4u-r-bom-body'),tot=document.getElementById('l4u-r-bom-total'),range=document.getElementById('l4u-r-budget-range');
    if(!body)return;
    ...
    if(range&&s.budget){
      ...
      range.innerHTML='<div style="background:linear-gradient(135deg,rgba(34,197,94,0.12),rgba(245,158,11,0.10));border:1px solid var(--border,#334155);border-radius:12px;padding:14px 16px;margin-top:4px;">'+
        ...
        '<div style="font-size:11px;color:var(--muted);margin-top:6px;line-height:1.5;">Range reflects market price variation across brands and suppliers. Your installer or vendor will provide an exact quote.</div>'+
      '</div>';
    }
  }
