// EG-VERSION: v304-fix
  ...
  function l4u__calcEarth(soilId, acCabS, kva){
    ...
    var rho = soil.rho;
    // Practical rod count — what installers actually do (Option A)
    var n = kva <= 5 ? 1 : kva <= 15 ? 2 : 3;
    // Calculate actual resistance achieved with n rods (IEEE 80 / BS 7430)
  ...
  function l4u__renderResults(s){
    ...
    var backupHrs = +((b0.totalKwh||0)*1000 / Math.max(s.tw,1)).toFixed(1);
    var coveragePct = Math.min(100,+((s.gen/(s.dw/1000))*100).toFixed(0));
    var friendlyBat = (b0.totalKwh||0)>=10 ? 'Large Home Battery ('+b0.totalKwh+'kWh)'
                    : (b0.totalKwh||0)>=5   ? 'Medium Home Battery ('+b0.totalKwh+'kWh)'
                    :                         'Home Battery ('+b0.totalKwh+'kWh)';
    var friendlyInv = s.inv.kva+'kVA Solar Power Unit';
    var priceLow  = Math.round(s.total * 0.85 / 1000) * 1000;
    var priceHigh = Math.round(s.total * 1.15 / 1000) * 1000;
    function fmtM(n){ return n>=1000000 ? '₦'+(n/1000000).toFixed(1)+'M' : '₦'+N(n); }
    var summaryEl = document.getElementById('l4u-r-plain-summary');
    if(summaryEl){
      summaryEl.innerHTML =
        '<div style="border:2px solid var(--sun,#f59e0b);border-radius:16px;padding:20px 16px;margin-bottom:8px;box-sizing:border-box;overflow:hidden;">'+
        '<div style="font-size:11px;text-transform:uppercase;letter-spacing:1px;color:var(--sun,#f59e0b);font-weight:700;margin-bottom:16px;">✅ Your Solar Recommendation</div>'+

        // Row 1: Inverter
        '<div style="border-bottom:1px solid rgba(255,255,255,0.08);padding-bottom:14px;margin-bottom:14px;">'+
          '<div style="font-size:11px;color:var(--muted,#94a3b8);margin-bottom:4px;text-transform:uppercase;letter-spacing:.5px;">Solar Power Unit</div>'+
          '<div style="font-size:20px;font-weight:900;color:var(--text,#f1f5f9);line-height:1.2;">'+friendlyInv+'</div>'+
        '</div>'+

        // Row 2: Battery
        '<div style="border-bottom:1px solid rgba(255,255,255,0.08);padding-bottom:14px;margin-bottom:14px;">'+
          '<div style="font-size:11px;color:var(--muted,#94a3b8);margin-bottom:4px;text-transform:uppercase;letter-spacing:.5px;">Battery Storage</div>'+
          '<div style="font-size:20px;font-weight:900;color:var(--text,#f1f5f9);line-height:1.2;">'+(b0.units||1)+' × '+friendlyBat+'</div>'+
        '</div>'+

        // Row 3: Panels
        '<div style="border-bottom:1px solid rgba(255,255,255,0.08);padding-bottom:14px;margin-bottom:14px;">'+
          '<div style="font-size:11px;color:var(--muted,#94a3b8);margin-bottom:4px;text-transform:uppercase;letter-spacing:.5px;">Solar Panels</div>'+
          '<div style="font-size:20px;font-weight:900;color:var(--text,#f1f5f9);line-height:1.2;">'+s.panels+' × Solar Panel (550W)</div>'+
        '</div>'+

        // Row 4: Key stats
        '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:16px;">'+
          '<div style="text-align:center;padding:12px 8px;background:rgba(34,197,94,0.1);border-radius:10px;">'+
            '<div style="font-size:22px;font-weight:900;color:#22c55e;line-height:1;">'+coveragePct+'%</div>'+
            '<div style="font-size:11px;color:var(--muted,#94a3b8);margin-top:4px;">Solar Cover</div>'+
          '</div>'+
          '<div style="text-align:center;padding:12px 8px;background:rgba(56,189,248,0.1);border-radius:10px;">'+
            '<div style="font-size:22px;font-weight:900;color:#38bdf8;line-height:1;">~'+backupHrs+'h</div>'+
            '<div style="font-size:11px;color:var(--muted,#94a3b8);margin-top:4px;">Backup Time</div>'+
          '</div>'+
          '<div style="text-align:center;padding:12px 8px;background:rgba(167,139,250,0.1);border-radius:10px;">'+
            '<div style="font-size:18px;font-weight:900;color:#a78bfa;line-height:1;word-break:break-all;">'+(s.dw/1000).toFixed(1)+'</div>'+
            '<div style="font-size:11px;color:var(--muted,#94a3b8);margin-top:4px;">kWh/Day</div>'+
          '</div>'+
        '</div>'+

        // Row 5: Price range
        '<div style="background:rgba(245,158,11,0.1);border-radius:12px;padding:16px;text-align:center;">'+
          '<div style="font-size:11px;color:var(--muted,#94a3b8);margin-bottom:6px;text-transform:uppercase;letter-spacing:.5px;">Estimated Equipment Cost</div>'+
          '<div style="font-size:26px;font-weight:900;color:var(--sun,#f59e0b);line-height:1;">'+fmtM(priceLow)+' – '+fmtM(priceHigh)+'</div>'+
          '<div style="font-size:12px;color:var(--muted,#94a3b8);margin-top:6px;">Price varies by vendor & location</div>'+
          '<div style="font-size:12px;color:var(--muted,#94a3b8);">Get exact quotes from vendors below</div>'+
        '</div>'+
    ...
    // Battery cards — use friendly label
    var bd=document.getElementById('l4u-r-battery-options');
    if(bd)bd.innerHTML=s.opts.map((o,i)=>{
      var fl = o.totalKwh>=10?'Large Home Battery ('+o.totalKwh+'kWh)':o.totalKwh>=5?'Medium Home Battery ('+o.totalKwh+'kWh)':'Home Battery ('+o.totalKwh+'kWh)';
      return '<div class="battery-card" id="ebo-'+i+'" style="cursor:pointer;border:2px solid '+(i===(s.selBat||0)?'var(--sun)':'var(--border)')+';transition:border-color 0.2s" onclick="l4u_egPickBat('+i+')">'+'<div class="btype">Option '+String.fromCharCode(65+i)+'</div>'+'<div class="bval" style="font-size:18px;">'+o.units+' ×</div>'+'<div style="font-size:11px;color:var(--sun);font-weight:700;margin:2px 0">'+fl+'</div>'+'<div class="bunit">'+o.totalKwh+' kWh total</div>'+'<div style="font-size:11px;color:var(--muted);margin-top:4px">'+o.wiring+'</div>'+'<div style="font-size:12px;color:var(--sun);font-weight:700;margin-top:6px">'+l4u__fmt(o.total)+'</div>'+(o.warn?'<div style="font-size:10px;color:#ef4444;margin-top:4px">⚠ '+o.warn+'</div>':'')+'</div>';
    }).join('');
    // Solar
    ...
    if(cab)cab.innerHTML=[['PV → Inverter (DC)',s.conn.pv],['Inverter → Battery (DC)',s.conn.bat],['Inverter → Changeover (AC)',s.conn.ac],['Grid → Inverter (AC)',s.conn.ac]].map(r=>'<tr style="border-top:1px solid var(--border,#334155)"><td style="padding:9px 8px;font-size:11px;color:var(--muted,#94a3b8);">'+r[0]+'</td><td style="padding:9px 4px;text-align:center;"><span style="background:#1e3a5f;color:#93c5fd;padding:3px 6px;border-radius:5px;font-size:11px;font-weight:600;">'+r[1].cab.s+'mm²</span></td><td style="padding:9px 4px;text-align:center;"><span style="background:#14532d;color:#86efac;padding:3px 6px;border-radius:5px;font-size:11px;font-weight:600;">'+r[1].brk.a+'A</span></td></tr>').join('');
    // SPD
    l4u__renderSPD(s);
    // Earthing soil selector + result
    l4u__renderSoilSelector();
    l4u__renderEarth(s);
    // BOM — update total to show range
    l4u__renderBOM(s);
    var tot=document.getElementById('l4u-r-bom-total');
    if(tot) tot.innerHTML = fmtM(priceLow)+' – '+fmtM(priceHigh);
    // Show
    ...
    /* Bridge to platform — sets userCalculationResult for Save/PDF/WhatsApp */
    try{
      if(typeof receiveEmbeddedCalculation==='function'){
        receiveEmbeddedCalculation('user', out, l4u_appliances);
      } else if(typeof handleUserCalcResult==='function'){
        handleUserCalcResult(out, l4u_appliances);
      }
  ...
  function l4u__onSoilChange(soilId){
    ...
    l4u__egS.earth = l4u__calcEarth(soilId, acCabS, l4u__egS.inv.kva);
    l4u__egS = l4u__buildBOM(l4u__egS, l4u__egS.selBat||0);
  ...
  window.l4u_toggleTechnical = function(){
    var el  = document.getElementById('l4u-technical-sections');
    var arr = document.getElementById('l4u-tech-toggle-arrow');
    var btn = document.getElementById('l4u-tech-toggle-btn');
    var lbl = document.getElementById('l4u-tech-toggle-label');
    if(!el) return;
    var open = el.style.display === 'block';
    el.style.display   = open ? 'none' : 'block';
    if(arr) arr.textContent  = open ? '▾' : '▴';
    if(lbl) lbl.textContent  = open ? '🔧 See technical details' : '🔧 Hide technical details';
    if(btn) btn.style.color  = open ? 'var(--muted,#94a3b8)' : 'var(--sun,#f59e0b)';
    if(btn) btn.style.borderColor = open ? 'var(--border,#334155)' : 'var(--sun,#f59e0b)';
  };
