...
// Remove the blur and show the technical specs section
function egApplyBlueprintUnlock() {
  const overlay = document.getElementById('eg-blueprint-paywall');
  if (overlay) overlay.remove();
  const specs   = document.getElementById('eg-technical-specs');
  if (specs) {
    specs.style.filter  = 'none';
    specs.style.pointerEvents = 'auto';
    specs.style.userSelect    = 'text';
  }
}

// Show the paywall overlay on the technical specs section
function egShowBlueprintPaywall(calcId) {
  ...
  // Blur the content
  specs.style.filter        = 'blur(6px)';
  specs.style.pointerEvents = 'none';
  specs.style.userSelect    = 'none';

  // Remove existing overlay if any
  const existing = document.getElementById('eg-blueprint-paywall');
  if (existing) existing.remove();

  const overlay = document.createElement('div');
  overlay.id = 'eg-blueprint-paywall';
  ...
  overlay.innerHTML = `
    ...
    <div style="font-weight:700;font-size:16px;margin-bottom:6px;color:#111827;">Technical Blueprint Locked</div>
    <div style="font-size:13px;color:#6b7280;margin-bottom:16px;max-width:260px;line-height:1.5;">
      Cable sizes (mm²), breaker ratings (A), wiring config &amp; PDF export.<br>
      One-time unlock — yours forever.
    </div>
    <button onclick="egPayForBlueprint('${calcId || ''}')"
    ...
    <div style="font-size:11px;color:#9ca3af;margin-top:10px;">
      Secure payment via Paystack · Bank transfer &amp; card accepted
    </div>
  `;

  // Position specs section relatively so overlay can use inset:0
  specs.style.position = 'relative';
  specs.appendChild(overlay);
}
