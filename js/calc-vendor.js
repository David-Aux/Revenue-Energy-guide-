  ...
  const l4v_PANEL = {wattage:550,vmp:41.8,imp:12.2,voc:49.5};
  ...
  const l4v_PC = {
    ...
    soil:[
      {id:'wet',   label:'Wet / Clay / Swampy', rho:50},
      {id:'loam',  label:'Loam / Mixed',         rho:100},
      {id:'sandy', label:'Sandy / Laterite',     rho:300},
      {id:'rocky', label:'Rocky / Dry Sandy',    rho:600},
    ],
  };
  ...
  function l4v__calcSPD(pps){
    var sv=pps*l4v_PANEL.voc, mu=sv*1.1;
    var dc=l4v_PC.dcSpd.find(function(s){return s.uc>=mu;})||l4v_PC.dcSpd[l4v_PC.dcSpd.length-1];
    return {dc:{uc:dc.uc,p:dc.p,stringVoc:+sv.toFixed(1),minUc:+mu.toFixed(1)},ac:{uc:l4v_PC.acSpd.uc,p:l4v_PC.acSpd.p}};
  }

  var l4v__ROD_L=3.0, l4v__ROD_D=0.016;
  var l4v__LAMBDA={1:1.0,2:1.16,3:1.29,4:1.36};
  ...
  function l4v__renderResults(s){
    ...
    // Vendor customer-needs summary
    l4v__renderCustomerNeeds(s);
    // BOM
    l4v__renderBOM(s);
    // Show
  ...
  function l4v__renderCustomerNeeds(s){
    var el=document.getElementById('l4v-r-customer-needs');
    if(!el)return;
    ...
    el.innerHTML=
      '<div style="border:2px solid #22c55e;border-radius:14px;padding:16px;margin-bottom:14px;background:rgba(34,197,94,0.06);">'
      +'<div style="font-size:10px;text-transform:uppercase;letter-spacing:1px;color:#22c55e;font-weight:700;margin-bottom:10px;">🛒 What This Customer Needs to Buy</div>'
      +'<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid var(--border,#334155);">'
        +'<div><div style="font-size:13px;font-weight:700;color:var(--text,#f1f5f9);">'+s.inv.kva+'kVA Hybrid Inverter @ '+s.V+'V</div>'
        +'<div style="font-size:11px;color:var(--muted,#94a3b8);">Solar Power Unit</div></div>'
        +'<div style="font-size:13px;font-weight:800;color:#22c55e;">×1</div>'
      +'</div>'
      +'<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid var(--border,#334155);">'
        +'<div><div style="font-size:13px;font-weight:700;color:var(--text,#f1f5f9);">'+(b0.label||'Battery')+'</div>'
        +'<div style="font-size:11px;color:var(--muted,#94a3b8);">LiFePO4 Battery Pack</div></div>'
        +'<div style="font-size:13px;font-weight:800;color:#22c55e;">×'+(b0.units||1)+'</div>'
      +'</div>'
      +'<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid var(--border,#334155);">'
        +'<div><div style="font-size:13px;font-weight:700;color:var(--text,#f1f5f9);">550W Mono PERC Solar Panel</div>'
        +'<div style="font-size:11px;color:var(--muted,#94a3b8);">'+s.strings+' string'+(s.strings>1?'s':'')+' × '+s.pps+' panels</div></div>'
        +'<div style="font-size:13px;font-weight:800;color:#22c55e;">×'+s.panels+'</div>'
      +'</div>'
      +'<div style="margin-top:12px;padding:12px;background:rgba(34,197,94,0.1);border-radius:10px;text-align:center;">'
        +'<div style="font-size:11px;color:var(--muted,#94a3b8);margin-bottom:4px;">ESTIMATED BUDGET RANGE</div>'
        +'<div style="font-size:20px;font-weight:900;color:#22c55e;">'+fmtM(priceLow)+' – '+fmtM(priceHigh)+'</div>'
      +'</div>'
      +'</div>';
  }
