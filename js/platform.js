    ...
    function egSyncUser(user) {
      currentUser = user;
      if (typeof egSetMonetizationUser === 'function') {
        egSetMonetizationUser(user);
        egRenderAdminBadge();
      }
    }
    ...
    function handleUserCalcResult(result, apps = []) {
      ...
      updateSavedSystemActionUI();
      showScreen('user-calculator');
      showToast('Sizing complete. Full result is below inside the calculator. Choose your next step when ready.', 'success');

      // ── Apply paywall to calculator cables section immediately ──
      try {
        const calcId = result._cloudId || null;
        const alreadyUnlocked = !!(result._isUnlocked);
        if (typeof egShowBlueprintPaywall === 'function') {
          egShowBlueprintPaywall(alreadyUnlocked ? null : calcId);
          if (alreadyUnlocked && typeof egApplyBlueprintUnlock === 'function') egApplyBlueprintUnlock();
        }
      } catch(e) { console.warn('Paywall init error:', e); }
    }
    ...
    function openUserCostScreen() {
      ...
      installerVendorBtns.forEach(btn => { btn.style.display = 'block'; });
      const backBtns = document.querySelectorAll('#user-cost-breakdown .btn-secondary');
      backBtns.forEach(btn => {
        btn.style.display = 'block';
        btn.textContent = '← Back to Calculator';
        btn.onclick = () => showScreen('user-calculator');
      });
      // ── Paywall: show blueprint gate (or unlock if already paid/admin) ──
      try {
        const calcId = userCalculationResult._cloudId || null;
        const alreadyUnlocked = !!(userCalculationResult._isUnlocked);
        if (alreadyUnlocked) {
          egApplyBlueprintUnlock();
        } else {
          egShowBlueprintPaywall(calcId);
        }
      } catch(e) { console.warn('Paywall init error:', e); }
    }
    ...
    function loadUserSavedCalculation(cloudId) {
      ...
      getSavedCalculationsSafe().then(calcs => {
        ...
        // Check if this saved calculation was already unlocked
        if (calc.is_unlocked) restored._isUnlocked = true;
...
// ================================================================
    ...
    async function renderJobLeads() {
      ...
      container.innerHTML = leads.map(lead => {
        ...
        const isUnlocked = unlockedLeadIds.has(lead.id) || isMine;
        ...
        let badgeClass = 'new';
        let badgeLabel = isUnlocked ? 'Unlocked' : 'Open';
        if (isMine && !isClosed) { badgeClass = 'contacted'; badgeLabel = 'Claimed by You'; }
        ...
        if (isUnlocked && !isMine && !isClosed) { badgeClass = 'contacted'; badgeLabel = 'Unlocked'; }
        ...
        if (isClosed) {
          ...
          buttons = `
            <button class="btn btn-success small-btn" onclick="egUnlockLead('${lead.id}', function(id){ renderJobLeads(); })">🔓 Unlock Lead — ₦2,000</button>`;
        } else if (isMine) {
          buttons = `
        ...
        <div class="lead-card" style="${isMine ? 'border-color: #22C55E; border-width: 2px;' : ''}">
          ...
          <div style="font-size: 13px; color: #687076; margin-bottom: 6px;">
            ${isMine ? `${phone || 'No phone'}${email ? ' &nbsp;·&nbsp; ' + email : ''}` : '📵 Phone visible after claiming &nbsp;·&nbsp; ✉️ Email visible after claiming'}${location ? ' &nbsp;·&nbsp; ' + location : ''}
          </div>
    ...
    async function renderVendorRequests() {
      ...
      container.innerHTML = data.map(lead => {
        ...
        const buttons = isClosed
          ? `<span style="font-size:12px;color:#687076;">This request is closed.</span>`
        ...
        <div class="lead-card">
          ...
          <div style="font-size: 13px; color: #687076; margin-bottom: 6px;">
            ${phone || 'No phone'}${location ? ' &nbsp;·&nbsp; ' + location : ''}
          </div>
          ${snapshot ? `<div style="font-size: 12px; color: #374151; margin-bottom: 6px;">${snapshot}</div>` : ''}
          ${extras   ? `<div style="font-size: 12px; color: #687076; margin-bottom: 6px;">${extras}</div>` : ''}
          ${lead.note ? `<div style="font-size: 12px; color: #687076; margin-bottom: 6px; font-style:italic;">"${lead.note}"</div>` : ''}
