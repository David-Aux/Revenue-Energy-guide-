// EG-VERSION: v304-fix
    ...
    async function egLoadLeadProductCheckboxes(vendorId) {
      ...
      try {
        ...
        if (error || !data || data.length === 0) {
          container.innerHTML = '<div style="color:#9ca3af;font-size:13px;font-style:italic;">No products listed by this vendor yet.</div>';
          return;
        }
    ...
    async function submitLead() {
      ...
      try {
        ...
        if (!isVendor) {
          ...
          if (snap && snap !== 'No calculation yet.') fullNote = (fullNote ? fullNote + '\n\n' : '') + '[System sizing]\n' + snap;
        }
    ...
    function downloadSystemPDF() {
      ...
      setTimeout(() => {
        try {
          ...
          // ── Build BOM from flat fields if r.bom is empty ──────────────────
          let bom = (r.bom && r.bom.length) ? r.bom : [];
          if (!bom.length) {
            ...
            if (r.pvCable)   addB('PV Cable DC',      r.pvCable   + 'mm\u00b2 DC ~20m', 20);
            if (r.battCable) addB('Battery Cable DC', r.battCable + 'mm\u00b2 DC ~2m',  2);
            if (r.acCable)   addB('AC Cable',         r.acCable   + 'mm\u00b2 AC ~20m', 20);
          ...
          // ── Cables & Breakers section ─────────────────────────────────────
          if (r.pvCable || r.battCable || r.acCable) {
            ...
            doc.setFontSize(10); doc.setTextColor(0);
            const connRows = [
              ['PV \u2192 Inverter (DC)',      r.pvCable   ? r.pvCable   + 'mm\u00b2 cable' : '-', r.pvBreaker   ? r.pvBreaker   + 'A MCB'  : '-'],
              ['Battery \u2192 Inverter (DC)', r.battCable ? r.battCable + 'mm\u00b2 cable' : '-', r.battBreaker ? r.battBreaker + 'A MCCB' : '-'],
              ['Inverter \u2192 AC Output',    r.acCable   ? r.acCable   + 'mm\u00b2 cable' : '-', r.acBreaker   ? r.acBreaker   + 'A MCB'  : '-'],
              ['Grid \u2192 Inverter (AC)',     r.acCable   ? r.acCable   + 'mm\u00b2 cable' : '-', r.acBreaker   ? r.acBreaker   + 'A MCB'  : '-'],
            ];
      ...
      const lines = [
        ...
        '\u2022 Battery: ' + kWh + 'kWh LiFePO4 Pack',
        '\u2022 Daily Energy: ' + Number(r.dailyKwh || 0).toFixed(2) + ' kWh/day',
        '\u2022 Running Load: ' + N(r.totalWatts) + 'W  |  Peak Surge: ' + N(r.maxSurge) + 'W',
      ];

      // ── Cables & Breakers (from flat fields set by platform.js calc) ──────
      const hasFlatConn = r.pvCable || r.battCable || r.acCable;
      if (hasFlatConn) {
        lines.push('', '*\ud83d\udd0c Cables & Breakers*');
        if (r.pvCable)   lines.push('\u2022 PV \u2192 Inverter (DC): ' + r.pvCable + 'mm\u00b2 cable | ' + (r.pvBreaker||'?') + 'A MCB');
        if (r.battCable) lines.push('\u2022 Battery \u2192 Inverter (DC): ' + r.battCable + 'mm\u00b2 cable | ' + (r.battBreaker||'?') + 'A MCCB');
        if (r.acCable)   lines.push('\u2022 AC Output/Grid: ' + r.acCable + 'mm\u00b2 cable | ' + (r.acBreaker||'?') + 'A MCB');
      }

      // ── SPD (from BOM) ────────────────────────────────────────────────────
      const spdItems = (r.bom || []).filter(b => b.item && b.item.includes('SPD'));
      if (spdItems.length) {
        lines.push('', '*\u26a1 Surge Protection (SPD)*');
        spdItems.forEach(s => lines.push('\u2022 ' + s.item + ': ' + s.spec));
      }

      // ── Earthing (from BOM) ───────────────────────────────────────────────
      const earthItems = (r.bom || []).filter(b => b.item && b.item.toLowerCase().includes('earth'));
      if (earthItems.length) {
        lines.push('', '*\ud83c\udf0d Earthing System*');
        earthItems.forEach(e => lines.push('\u2022 ' + e.item + ' (' + e.spec + ') \u00d7' + e.qty));
      }
      ...
      if (bomLines.length) {
        ...
        bomLines.forEach(l => lines.push(l));
      }
