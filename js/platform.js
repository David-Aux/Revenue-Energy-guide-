    ...
    async function egLoadAdminPanel() {
      ...
      el.innerHTML = data.map(p => {
        ...
        const actionBtns = statusVal === 'approved'
          ? '<button class="btn btn-danger" style="width:100%;" onclick="egAdminRejectProfile('' + escapeHtml(p.id) + '','' + escapeHtml(p.role||'') + '')">❌ Revoke Access</button>'
          : statusVal === 'rejected'
          ? '<button class="btn btn-success" style="width:100%;" onclick="egAdminApproveProfile('' + escapeHtml(p.id) + '','' + escapeHtml(p.role||'') + '')">✅ Approve</button>'
  ...
  try {
    ...
    async function signInWithGoogle(role) {
      try {
        ...
        // Write to BOTH localStorage AND the redirectTo URL as a query param,
        // because Android browsers (Samsung Internet, Chrome) can wipe localStorage
        // across OAuth redirects when storage partitioning is active.
        localStorage.setItem('pendingRole', role);

        const redirectTo = window.location.origin + window.location.pathname + '?pendingRole=' + encodeURIComponent(role);
    ...
    async function handleGoogleAuthReturn() {
      try {
        ...
        // Priority: URL param (most reliable on Android) > localStorage > metadata > profile > default
        const _urlParams = new URLSearchParams(window.location.search);
        const _urlPendingRole = _urlParams.get('pendingRole');
        if (_urlPendingRole) localStorage.setItem('pendingRole', _urlPendingRole); // re-sync storage from URL
        const _rawRole = _urlPendingRole
