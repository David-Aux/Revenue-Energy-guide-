  ...
  try {
    ...
    async function signInWithGoogle(role) {
      try {
        ...
        // Store pendingRole in THREE places to survive Android OAuth redirects:
        // 1. localStorage (primary)
        // 2. sessionStorage (backup)
        // 3. redirectTo URL query param (most reliable — survives all storage wipes)
        // Also stamp a timestamp so stale values don't persist across sessions.
        ...
        const redirectTo = window.location.origin + window.location.pathname + '?egpr=' + encodeURIComponent(role);
    ...
    async function handleGoogleAuthReturn() {
      try {
        ...
        // Determine the role — read from all sources in priority order:
        // 1. URL query param ?egpr= (most reliable, survives Android storage wipes)
        // 2. sessionStorage egPendingRole (survives same-tab redirects)
        // 3. localStorage egPendingRole (survives cross-tab)
        // 4. Legacy localStorage pendingRole (backwards compat)
        // 5. user_metadata.role (set at email signup)
        // 6. existing profile.role (returning user)
        // 7. 'user' fallback
        const _urlParams = new URLSearchParams(window.location.search);
        const _urlRole   = _urlParams.get('egpr') || _urlParams.get('pendingRole');
        ...
        function _readStoredRole(storage) {
          try {
            ...
            if (Date.now() - parsed.ts > 10 * 60 * 1000) { storage.removeItem('egPendingRole'); return null; }
