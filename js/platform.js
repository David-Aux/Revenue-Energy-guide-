// EG-VERSION: v304-fix
    ...
    function instShareWhatsApp() {
      ...
      const r = instCalculationResult;
      const N = egFormatNumber;
      const fmtM = v => 'NGN ' + Number(Math.round((v||0)/1000)).toLocaleString('en-NG') + 'k';
      const inv = Math.ceil((r.requiredInverter || 0) / 1000);
      const budMin = r.budgetMin || r.totalCost || 0;
      const budMax = r.budgetMax || 0;
      const rangeStr = budMax > budMin ? fmtM(budMin) + ' – ' + fmtM(budMax) : fmtM(budMin);
      ...
      const msg = egJoinLines([
        ...
        '',
        '*Estimated System Cost: ' + rangeStr + '*',
        '',
        '_Range reflects market price variation. Installer will quote exact price._',
        '_Generated by Energy Guide_',
      ]));
      egOpenWhatsApp(msg);
    }
    ...
    function downloadSystemPDF() {
      ...
      setTimeout(() => {
        try {
          ...
          doc.text('Estimated System Cost',14,y);
          const budMin=r.budgetMin||r.totalCost||0, budMax=r.budgetMax||0;
          const rangeStr=budMax>budMin?fmtM(budMin)+' – '+fmtM(budMax):fmtM(budMin);
          doc.setFontSize(14); doc.setFont(undefined,'bold');
          doc.text(rangeStr,196,y,{align:'right'});
          doc.setFont(undefined,'normal'); y+=8;
          doc.setFontSize(8); doc.setTextColor(130);
          const disc='This range reflects market price variation across brands and suppliers in Nigeria. Your installer or vendor will provide an exact itemised quote. Prices are indicative only and subject to change based on current FX rates and availability.';
      ...
      const msg = [
        ...
        '*Estimated System Cost: ' + rangeStr + '*',
        '',
        '_Range reflects market price variation across brands and suppliers._',
        '_Your installer or vendor will provide an exact quote._',
        '_Generated by Energy Guide_',
      ]).join('\n');
