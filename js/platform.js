    ...
    // Sync user object into currentUser and persist to localStorage
    function egSyncUser(userObj) {
      if (!userObj) return;
      currentUser = { ...userObj };
      try { localStorage.setItem('eg_current_user', JSON.stringify(currentUser)); } catch(e) {}
    }
    ...
    async function egLoadVendorProductsInline(vendorId) {
      ...
      try {
        const { data, error } = await supabaseClient
          ...
          .select('id, name, category, brand, power_watts, price, quantity, image_url, description')
          .eq('user_id', vendorId)
          .order('created_at', { ascending: false });
        if (error) {
          console.error('egLoadVendorProductsInline error:', error);
          container.innerHTML = '<div style="font-size:12px;color:#ef4444;">Could not load products. (' + (error.message || 'RLS or network error') + ')</div>';
          return;
        }
        ...
        const SHOW_LIMIT = 5;
        const typeLabels = { 'Solar Panel':'Solar Panel', 'Inverter':'Inverter', 'Battery':'Battery', 'Charge Controller':'Controller', 'Cable':'Cable', 'Breaker':'Breaker' };

        // Store product data in window map keyed by product id for safe onclick access
        if (!window._egProductCache) window._egProductCache = {};
        data.forEach(p => { window._egProductCache[p.id] = { ...p, _vendorId: vendorId }; });

        function buildProductCard(p) {
          const nameSafe = safeText(p.name || 'Unnamed product');
          const catLabel = safeText(typeLabels[p.category] || p.category || '');
          const subtitle = [catLabel, p.power_watts ? p.power_watts+'W' : '', p.brand ? safeText(p.brand) : ''].filter(Boolean).join(' · ');
          const imgHtml = p.image_url
            ? `<img src="${p.image_url}" style="width:48px;height:48px;object-fit:cover;border-radius:6px;flex-shrink:0;" onerror="this.style.display='none'">`
            : `<div style="width:48px;height:48px;border-radius:6px;background:#fef3c7;display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0;">📦</div>`;
          return `
            <div onclick="egShowProductDetail('${vendorId}','${p.id}')"
              style="display:flex;gap:10px;align-items:flex-start;padding:10px 0;
                     border-bottom:1px solid #f3f4f6;cursor:pointer;"
              ontouchstart="this.style.background='#f9fafb'"
              ontouchend="this.style.background=''">
              ${imgHtml}
              <div style="flex:1;min-width:0;">
                <div style="font-weight:600;font-size:13px;">${nameSafe}</div>
                <div style="color:#6b7280;font-size:12px;">${subtitle}</div>
                ${p.description ? `<div style="color:#9ca3af;font-size:11px;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${safeText(p.description)}</div>` : ''}
              </div>
              <div style="text-align:right;flex-shrink:0;margin-left:8px;">
                <div style="font-weight:700;color:#16a34a;font-size:14px;">₦${Number(p.price||0).toLocaleString()}</div>
                ${p.quantity ? `<div style="font-size:11px;color:#9ca3af;">Qty: ${p.quantity}</div>` : ''}
                <div style="font-size:11px;color:#F59E0B;margin-top:2px;font-weight:600;">Tap to enquire →</div>
              </div>
            </div>`;
        }

        const visible = data.slice(0, SHOW_LIMIT);
        const hidden  = data.slice(SHOW_LIMIT);
        const hiddenId = `egProdHidden_${vendorId}`;
        const btnId    = `egProdShowAll_${vendorId}`;

        let html = visible.map(buildProductCard).join('');

        if (hidden.length > 0) {
          html += `<div id="${hiddenId}" style="display:none;">${hidden.map(buildProductCard).join('')}</div>`;
          html += `<button id="${btnId}" onclick="
            document.getElementById('${hiddenId}').style.display='block';
            this.style.display='none';
          " style="margin-top:10px;width:100%;padding:8px;border:1px solid #e5e7eb;
                   border-radius:8px;background:#f9fafb;font-size:12px;
        ...
        console.error('egLoadVendorProductsInline exception:', e);
      }
    }

    // ── Product detail sheet ──────────────────────────────────
    function egShowProductDetail(vendorId, productId) {
      const p = (window._egProductCache && window._egProductCache[productId]);
      if (!p) { showToast('Could not load product details.', 'error'); return; }
      const nameSafe = safeText(p.name || 'Product');
      const typeLabels = { 'Solar Panel':'Solar Panel', 'Inverter':'Inverter', 'Battery':'Battery', 'Charge Controller':'Controller', 'Cable':'Cable', 'Breaker':'Breaker' };
      const catLabel = typeLabels[p.category] || p.category || '';
      const subtitle = [catLabel, p.power_watts ? p.power_watts+'W' : '', p.brand ? p.brand : ''].filter(Boolean).join(' · ');

      const imgHtml = p.image_url
        ? `<img src="${p.image_url}" style="width:100%;max-height:200px;object-fit:cover;border-radius:12px;margin-bottom:16px;" onerror="this.style.display='none'">`
        : `<div style="width:100%;height:120px;border-radius:12px;background:#fef3c7;display:flex;align-items:center;justify-content:center;font-size:48px;margin-bottom:16px;">📦</div>`;

      egShowModal(`
        <div>
          ${imgHtml}
          <div style="font-weight:800;font-size:20px;color:#111827;margin-bottom:4px;">${nameSafe}</div>
          ${subtitle ? `<div style="font-size:13px;color:#6b7280;margin-bottom:8px;">${safeText(subtitle)}</div>` : ''}
          <div style="font-size:26px;font-weight:800;color:#16a34a;margin-bottom:12px;">
            ₦${Number(p.price||0).toLocaleString()}
          </div>
          ${p.quantity ? `<div style="font-size:13px;color:#6b7280;margin-bottom:8px;">Available Qty: <strong>${p.quantity}</strong></div>` : ''}
          ${p.description ? `<div style="font-size:13px;color:#374151;line-height:1.6;margin-bottom:16px;padding:12px;background:#f9fafb;border-radius:8px;">${safeText(p.description)}</div>` : ''}
          <button onclick="egRemoveModal(); openProductEnquiryFromCache('${vendorId}','${productId}')"
            style="width:100%;background:#F59E0B;color:#fff;border:none;border-radius:14px;
                   ...
                   box-shadow:0 4px 16px rgba(245,158,11,0.4);margin-bottom:10px;">
        ...
        return `
            <div class="result-card" style="text-align:left; margin-bottom: 14px;">
              ...
              <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-top:14px;">
                ...
                <button class="btn btn-success" onclick="openInstallerEnquiry('${safeText(profile.id)}')">Request Quote</button>
              </div>
            </div>`;
      }).join('');
    }
        ...
        return `
            <div class="result-card" style="text-align:left; margin-bottom: 14px;">
              ...
              <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-top:14px;">
                ...
                <button class="btn btn-primary" onclick="openVendorEnquiry('${safeText(profile.id)}')">Contact Vendor</button>
              </div>
            </div>`;
      }).join('');
    }
    ...
    // ── Request Quote from installer (cold or post-calc) ──────
    async function openInstallerEnquiry(installerId) {
      _pendingProductEnquiry = null;
      selectedInstallerId = installerId;
      selectedVendorId = null;
      selectedVendorProfile = null;
      try {
        const { data } = await supabaseClient.from('profiles').select('*').eq('id', installerId).maybeSingle();
        if (data) selectedInstallerProfile = data;
      } catch (e) { console.warn('Failed to load installer profile:', e); }
      const marketplaceState = document.getElementById('installerMarketplaceState');
      const leadState = document.getElementById('leadState');
      if (marketplaceState && leadState && marketplaceState.value) leadState.value = marketplaceState.value;
      currentLeadTarget = 'installer';
      const titleEl   = document.getElementById('leadFlowTitle');
      const subtitleEl = document.getElementById('leadFlowSubtitle');
      if (titleEl)    titleEl.textContent   = 'Request Quote';
      if (subtitleEl) subtitleEl.textContent = selectedInstallerProfile
        ? `Request a quote from ${selectedInstallerProfile.business_name || selectedInstallerProfile.company_name || selectedInstallerProfile.full_name || 'this installer'}.`
        : 'Fill in your details and the installer will get back to you.';
      // Show snapshot only if a calculation exists
      egSetLeadFormMode(userCalculationResult ? 'installer' : 'installer-cold');
    ...
    // ── Form field visibility helper ─────────────────────────────
    function egSetLeadFormMode(mode, productName) {
      // mode: 'installer' | 'vendor' | 'product'
      ...
      const snapshotSection  = document.getElementById('leadSnapshotSection');
      const productBanner    = document.getElementById('leadProductBanner');
      const productBannerName= document.getElementById('leadProductBannerName');
      const noteField        = document.getElementById('leadNote');
      const noteLabel        = document.getElementById('leadNoteLabel');

      if (mode === 'installer' || mode === 'installer-cold') {
        if (installerFields)  installerFields.style.display  = '';
        if (vendorFields)     vendorFields.style.display     = 'none';
        // Show snapshot only when user has a calculation (post-calc flow)
        if (snapshotSection)  snapshotSection.style.display  = (mode === 'installer' && userCalculationResult) ? '' : 'none';
        if (productBanner)    productBanner.style.display    = 'none';
        if (noteLabel)        noteLabel.textContent = 'Installation Note';
        if (noteField)        noteField.placeholder = 'Any special note, roof info, timing or access note';
      } else if (mode === 'vendor') {
        if (installerFields)  installerFields.style.display  = 'none';
        if (vendorFields)     vendorFields.style.display     = '';
        if (snapshotSection)  snapshotSection.style.display  = 'none';
        if (productBanner)    productBanner.style.display    = 'none';
        if (noteLabel)        noteLabel.textContent = 'Note to Vendor';
        if (noteField)        noteField.placeholder = 'Any specific product, brand, quantity or delivery note';
      } else if (mode === 'product') {
        if (installerFields)  installerFields.style.display  = 'none';
        if (vendorFields)     vendorFields.style.display     = 'none';
        if (snapshotSection)  snapshotSection.style.display  = 'none';
        if (productBanner)    productBanner.style.display    = '';
        if (productBannerName && productName) productBannerName.textContent = productName;
        if (noteLabel)        noteLabel.textContent = 'Note to Vendor';
        if (noteField)        noteField.placeholder = 'Quantity needed, delivery address, or any question about this product';
      }
    }

    // ── Contact Vendor (no calculation required) ──────────────
    let _pendingProductEnquiry = null; // { id, name } or null

    async function openVendorEnquiry(vendorId) {
      _pendingProductEnquiry = null;
      selectedVendorId = vendorId;
      selectedInstallerId = null;
      selectedInstallerProfile = null;
      try {
        const { data } = await supabaseClient.from('profiles').select('*').eq('id', vendorId).maybeSingle();
        if (data) selectedVendorProfile = data;
      } catch (e) { console.warn('Failed to load vendor profile:', e); }
      const marketplaceState = document.getElementById('vendorMarketplaceState');
      const leadState = document.getElementById('leadState');
      if (marketplaceState && leadState && marketplaceState.value) leadState.value = marketplaceState.value;
      // Update form title for general vendor enquiry
      currentLeadTarget = 'vendor';
      const titleEl = document.getElementById('leadFlowTitle');
      const subtitleEl = document.getElementById('leadFlowSubtitle');
      ...
      const noteLabel = document.getElementById('leadNoteLabel');
      const noteField = document.getElementById('leadNote');
      if (titleEl) titleEl.textContent = 'Contact Vendor';
      if (subtitleEl) subtitleEl.textContent = 'Fill in your details and the vendor will reach out to you.';
      egSetLeadFormMode('vendor');
      showScreen('submit-lead');
    }

    // ── Product Enquiry from cache (called from modal button) ─
    async function openProductEnquiryFromCache(vendorId, productId) {
      const p = (window._egProductCache && window._egProductCache[productId]);
      const productName = p ? (p.name || 'Product') : 'Product';
      await openProductEnquiry(vendorId, productId, productName);
    }

    // ── Product Enquiry ───────────────────────────────────────
    async function openProductEnquiry(vendorId, productId, productName) {
      _pendingProductEnquiry = { id: productId, name: productName };
      selectedVendorId = vendorId;
      selectedInstallerId = null;
      selectedInstallerProfile = null;
      try {
      ...
      const noteLabel = document.getElementById('leadNoteLabel');
      const noteField = document.getElementById('leadNote');
      if (titleEl) titleEl.textContent = 'Enquire About Product';
      if (subtitleEl) subtitleEl.textContent = 'Fill in your details and the vendor will contact you about this product.';
      egSetLeadFormMode('product', productName);
      showScreen('submit-lead');
    }
    ...
    async function submitLead() {
      ...
      // Only require project type for installer flow
      const isVendorFlow = currentLeadTarget === 'vendor';
      if (!projectType && !isVendorFlow) setErr('leadProjectTypeError', 'Project type is required');

      if (!valid) { showToast('Please fill in all required fields', 'error'); return; }
      // Sync vendor need dropdown → vendorQuoteType field
      const vendorNeedEl = document.getElementById('leadVendorNeed');
      const vendorQTEl   = document.getElementById('leadVendorQuoteType');
      if (vendorNeedEl && vendorQTEl && vendorNeedEl.value) vendorQTEl.value = vendorNeedEl.value;

      // Allow submission without a calculation — sizing fields will be null
      ...
      const lead = {
        ...
        product_enquiry_name: (isVendorLead && _pendingProductEnquiry) ? _pendingProductEnquiry.name : null,
      ...
      if (!userCalculationResult) { showToast('No sizing yet — your request will be sent without a system snapshot.', 'info'); }
      currentLeadTarget = target;
      egSetLeadFormMode(target === 'vendor' ? 'vendor' : 'installer'); // post-calc: snapshot visible for installer
      const titleEl = document.getElementById('leadFlowTitle');
      const subtitleEl = document.getElementById('leadFlowSubtitle');
...
// ================================================================
    ...
    async function renderVendorRequests() {
      ...
      const vendorState = await getCurrentUserPortalState('vendor');
      // Show leads targeted at this vendor OR general vendor leads in their state
      const vendorUserId2 = await getCurrentAuthUserId().catch(() => null);
      let query = supabaseClient
        .from('leads')
        .select('*')
        .in('status', ['vendor_requested', 'vendor_responded', 'closed']);
      if (vendorUserId2) {
        // targeted leads for this vendor + general leads in their state
        query = query.or(`target_vendor_id.eq.${vendorUserId2},target_vendor_id.is.null`);
      }
      if (vendorState) query = query.eq('state', vendorState);
      ...
      const vendorUserId = vendorUserId2 || await getCurrentAuthUserId().catch(() => null);
      ...
      container.innerHTML = data.map(lead => {
        ...
        const extras = [
          lead.product_enquiry_name ? `Product: <strong>${safeText(lead.product_enquiry_name)}</strong>` : null,
    ...
    async function handleGoogleAuthReturn() {
      try {
        ...
        if (hasHashTokens) {
          ...
          if (error) {
            // Code already used or expired — try getting existing session instead
        ...
        currentUser = { ...session.user, ...(activeProfile || {}) };
