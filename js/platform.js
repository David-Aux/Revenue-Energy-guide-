    ...
    async function saveCompleteInstallerProfile() {
      ...
      try {
        ...
        showLoading(false);
        if (error) throw error;
        currentUser = { ...currentUser, company_name: business || display_name, phone, state, city, status: 'pending' };
    ...
    async function handleInstallerLogin() {
      ...
      if (result.success && isInstaller) {
        ...
        if (instProfile.status === 'pending') {
          showToast('Welcome back, ' + displayName + '! Your application is under review.', 'info');
        } else if (instProfile.status === 'rejected') {
          showToast('Your application was not approved. Contact us for more info.', 'error');
        } else {
        ...
        showToast('This account is registered as a vendor. Please use the Vendor Portal to log in.', 'error');
      ...
      if (result.success && isVendor) {
        ...
        if (vendProfile.status === 'pending') {
          showToast('Welcome back, ' + displayName + '! Your application is under review.', 'info');
        } else if (vendProfile.status === 'rejected') {
          showToast('Your application was not approved. Contact us for more info.', 'error');
        } else {
          showToast('Welcome back, ' + displayName + '!', 'success');
        }
        ...
        showToast('This account is registered as an installer. Please use the Installer Portal to log in.', 'error');
...
async function supabaseSubmitMarketplaceLead(lead) {
  try {
    ...
    async function registerVendor() {
      ...
      if (result.success) {
        currentUser = { ...(result.user || {}), role: 'vendor', company_name: company, full_name: fullName, phone, state, city, status: 'pending' };
        currentRole = 'vendor';
        ...
        if (isExisting) {
          document.getElementById('regVendorEmailError').textContent = 'Email already registered — please log in instead';
          showToast('This email already has an account. Use Sign In below.', 'info');
        } else {
    ...
    async function registerInstaller() {
      ...
      if (result.success) {
        currentUser = { ...(result.user || {}), role: 'installer', company_name: company, full_name: fullName, phone, state, city: area, status: 'pending' };
        currentRole = 'installer';
        ...
        const isExisting = result.error && (result.error.includes('already') || result.error.includes('confirmation') || result.error === 'already_registered');
        if (isExisting) {
          document.getElementById('regInstEmailError').textContent = 'Email already registered — please log in instead';
          showToast('This email already has an account. Use Sign In below.', 'info');
        } else {
          showToast(result.error || 'Signup failed', 'error');
        }
