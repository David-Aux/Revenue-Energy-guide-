    ...
    function egShowUserEmailConfirmState(email) {
      // Hide the form fields, show the confirmation state inside the modal
      const formEls = ['userAuthFullNameGroup', 'userAuthSubmitBtn'];
      document.querySelectorAll('#userAuthModal .form-group').forEach(el => el.style.display = 'none');
      ['userAuthTabSignup','userAuthTabSignin','userAuthSubmitBtn'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.style.display = 'none';
      });
      // Hide the Google/Maybe Later buttons row
      const btnRow = document.querySelector('#userAuthModal [style*="flex-wrap:wrap"]');
      if (btnRow) btnRow.style.display = 'none';
      const confirmState = document.getElementById('userEmailConfirmState');
      const emailEl = document.getElementById('userConfirmEmailAddress');
      if (confirmState) confirmState.style.display = 'block';
      if (emailEl && email) emailEl.textContent = email;
      if (email) localStorage.setItem('pendingConfirmEmail', email);
    }

    async function egResendUserConfirmationEmail() {
      const email = localStorage.getItem('pendingConfirmEmail')
        || (document.getElementById('userConfirmEmailAddress') || {}).textContent
        || (document.getElementById('userAuthEmail') || {}).value;
      if (!email) { showToast('Email not found. Please try signing up again.', 'error'); return; }
      if (!supabaseClient) { showToast('Not connected', 'error'); return; }
      showLoading(true, 'Resending...');
      try {
        ...
        showLoading(false);
        showToast('Could not resend confirmation email.', 'error');
      }
    }

    function egShowEmailConfirmationBanner(email) {
      const banner = document.getElementById('pendingEmailBanner');
      const emailEl = document.getElementById('pendingEmailAddress');
      if (banner) banner.style.display = 'block';
      if (emailEl && email) emailEl.textContent = email;
      // Store email so resend button can use it
      if (email) localStorage.setItem('pendingConfirmEmail', email);
    }

    async function egResendConfirmationEmail() {
      const email = localStorage.getItem('pendingConfirmEmail');
      if (!email) {
      ...
      if (!supabaseClient) { showToast('Not connected', 'error'); return; }
      showLoading(true, 'Resending confirmation...');
      try {
        const { error } = await supabaseClient.auth.resend({
          type: 'signup',
          email: email,
      ...
      return `
        <div class="result-card" style="text-align:left; margin-bottom:14px; border-left:4px solid #22C55E; padding:20px;">
          ...
          <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-top:4px;">
            <a href="${mailLink}" class="btn btn-outline" style="text-decoration:none; display:flex; align-items:center; justify-content:center; gap:6px; font-size:14px;">✉️ Email Us</a>
            <a href="${waLink}" target="_blank" class="btn btn-success" style="text-decoration:none; display:flex; align-items:center; justify-content:center; gap:6px; font-size:14px;">💬 WhatsApp</a>
          </div>
        </div>`;
    }
    ...
    function egTeamVendorCard(state) {
      ...
      return `
        <div class="result-card" style="text-align:left; margin-bottom:14px; border-left:4px solid #F59E0B; padding:20px;">
          ...
          <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-top:4px;">
            <a href="${mailLink}" class="btn btn-outline" style="text-decoration:none; display:flex; align-items:center; justify-content:center; gap:6px; font-size:14px;">✉️ Email Us</a>
            <a href="${waLink}" target="_blank" class="btn btn-warning" style="text-decoration:none; display:flex; align-items:center; justify-content:center; gap:6px; font-size:14px;">💬 WhatsApp</a>
          </div>
        </div>`;
    }
    ...
    async function handleUserAuthSubmit() {
      ...
      if (userAuthMode === 'signup' && result.emailConfirmationRequired) {
        // Don't close the modal — show persistent confirmation state inside it
        egShowUserEmailConfirmState(email);
        return;
      }
  ...
  try {
    ...
    const { error: profileError } = _safeRole ? await supabaseClient
      ...
      .upsert([{
        ...
        works_with_tubular: userData.works_with_tubular || false,
        ...
        sells_tubular: userData.sells_tubular || false,
    ...
    async function registerVendor() {
      ...
      if (result.success) {
        ...
        if (result.emailConfirmationRequired) {
          egShowEmailConfirmationBanner(email);
        }
      } else {
    ...
    async function handleGoogleAuthReturn() {
      try {
        ...
        if (isEmailConfirmation) {
          showToast('Email confirmed! Welcome to Energy Guide.', 'success');
        } else {
