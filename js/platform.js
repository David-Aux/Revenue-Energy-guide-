    ...
    function downloadSystemPDF() {
      ...
      setTimeout(() => {
        try {
          ...
          const batDesc = r.batLabel ? (r.batUnits || 1) + '\u00d7 ' + r.batLabel : kWh + 'kWh LiFePO4 Pack';
          ...
          doc.text('Solar System Specification Report', 105, 27, { align: 'center' });
          ...
          // ── Section 1: Appliance Load Profile ────────────────────────────
          if (apps.length > 0) {
            doc.setFontSize(12); doc.setTextColor(10, 126, 164);
            doc.text('1. Appliance Load Profile', 14, y); y += 7;

            // Table header
            doc.setFontSize(8.5);
            doc.setFillColor(10, 126, 164); doc.rect(14, y - 4, 182, 6.5, 'F');
            doc.setTextColor(255); doc.setFont(undefined, 'bold');
            doc.text('Appliance', 17, y);
            doc.text('Watts', 90, y, { align: 'right' });
            doc.text('Qty', 115, y, { align: 'right' });
            doc.text('Hrs/Day', 145, y, { align: 'right' });
            doc.text('Daily Wh', 192, y, { align: 'right' });
            doc.setFont(undefined, 'normal'); y += 3;

            let totalDailyWh = 0;
            apps.forEach(function (a, i) {
              if (y > 265) { doc.addPage(); y = 20; }
              const hrs = a.dutyCycle ? (24 * a.dutyCycle).toFixed(1) + 'h*' : (a.hours || 0) + 'h';
              const dailyWh = Math.round(a.watts * a.qty * (a.dutyCycle ? 24 * a.dutyCycle : (a.hours || 0)));
              totalDailyWh += dailyWh;
              if (i % 2 === 0) { doc.setFillColor(245, 249, 255); doc.rect(14, y - 3, 182, 6, 'F'); }
              doc.setTextColor(30); doc.text(String(a.name || ''), 17, y, { maxWidth: 68 });
              doc.setTextColor(80);
              doc.text(N(a.watts) + 'W', 90, y, { align: 'right' });
              doc.text(String(a.qty || 1), 115, y, { align: 'right' });
              doc.text(hrs, 145, y, { align: 'right' });
              doc.text(N(dailyWh) + 'Wh', 192, y, { align: 'right' });
              y += 6;
            });

            // Totals row
            if (y > 265) { doc.addPage(); y = 20; }
            doc.setFillColor(230, 240, 255); doc.rect(14, y - 3, 182, 6.5, 'F');
            doc.setTextColor(10, 126, 164); doc.setFont(undefined, 'bold');
            doc.text('Total Daily Consumption', 17, y);
            doc.text(N(totalDailyWh) + ' Wh/day', 192, y, { align: 'right' });
            doc.setFont(undefined, 'normal');
            y += 5;

            doc.setFontSize(7.5); doc.setTextColor(140);
            doc.text('* Duty cycle applied — fridge/freezer runs intermittently, not full hours', 17, y);
            y += 8;
          }

          // ── Section 2: Recommended System ────────────────────────────────
          if (y > 240) { doc.addPage(); y = 20; }
          doc.setLineWidth(0.3); doc.setDrawColor(200); doc.line(14, y, 196, y); y += 6;
          doc.setFontSize(12); doc.setTextColor(10, 126, 164);
          doc.text('2. Recommended System', 14, y); y += 7;
          doc.setFontSize(10); doc.setTextColor(30);

          const fields = [
            ['Inverter',      (r.invKva || '-') + 'kVA Hybrid MPPT @ ' + (r.sysV || '-') + 'V DC'],
            ['Solar Panels',  (r.numPanels || 0) + ' \u00d7 550W Mono PERC  (' + N((r.numPanels || 0) * 550) + 'W array)'],
            ['Battery Bank',  batDesc],
            ['Running Load',  N(r.totalWatts) + 'W'],
            ['Peak Surge',    N(r.maxSurge) + 'W'],
            ['Daily Energy',  Number(r.dailyKwh || 0).toFixed(2) + ' kWh/day'],
          ];
          fields.forEach(function (f) {
            if (y > 265) { doc.addPage(); y = 20; }
            doc.setTextColor(100); doc.text(f[0] + ':', 18, y);
            doc.setTextColor(30);  doc.text(f[1], 70, y);
            y += 6;
          });
          ...
          if ((r.sysV || 0) >= 48) {
            const noteLines = doc.splitTextToSize('System voltage set to 48V — load exceeds 3kW threshold, higher voltage improves efficiency and reduces cable losses.', 178);
            doc.text(noteLines, 18, y); y += noteLines.length * 4 + 2;
          }
          if ((r.maxSurge || 0) > (r.totalWatts || 0) * 2) {
            const surgeLines = doc.splitTextToSize('High surge load detected — inverter sized to handle motor start-up peaks, not just running load.', 178);
            doc.text(surgeLines, 18, y); y += surgeLines.length * 4 + 2;
          }
          y += 4;
          ...
          doc.setLineWidth(0.3); doc.setDrawColor(200); doc.line(14, y, 196, y); y += 6;
          doc.setFontSize(12); doc.setTextColor(10, 126, 164);
          doc.text('3. Bill of Quantities', 14, y); y += 7;
          ...
          doc.text('Qty', 192, y, { align: 'right' });
          doc.setFont(undefined, 'normal'); y += 3;

          (r.bom || []).forEach(function (row, i) {
            if (y > 265) { doc.addPage(); y = 20; }
            if (i % 2 === 0) { doc.setFillColor(248, 250, 252); doc.rect(14, y - 3, 182, 6, 'F'); }
            doc.setTextColor(30);  doc.text(row.item || '', 17, y, { maxWidth: 58 });
            doc.setTextColor(90);  doc.text(row.spec || '', 80, y, { maxWidth: 105 });
            doc.setTextColor(30);  doc.text(String(row.qty || ''), 192, y, { align: 'right' });
            y += 6;
          });
          y += 6;
          ...
          doc.setLineWidth(0.3); doc.setDrawColor(200); doc.line(14, y, 196, y); y += 5;
          doc.setFontSize(8); doc.setTextColor(130);
          const disc = 'Pricing is not included in this report. Contact a certified installer or equipment vendor through Energy Guide for an exact quote with current market pricing.';
          const dlines = doc.splitTextToSize(disc, 182);
          doc.text(dlines, 14, y); y += dlines.length * 4 + 4;

          // ── Footer ────────────────────────────────────────────────────────
          doc.setFontSize(8); doc.setTextColor(160);
          doc.text('Generated by Energy Guide  |  energyguide.app  |  ' + dateStr, 105, 288, { align: 'center' });
      ...
      // Appliance list
      const appLines = apps.length > 0
        ? apps.map(a => {
            const hrs = a.dutyCycle ? (24 * a.dutyCycle).toFixed(1) + 'h (duty cycle)' : (a.hours || 0) + 'h/day';
            return '  • ' + (a.name || 'Appliance') + ' — ' + (a.watts || 0) + 'W \u00d7' + (a.qty || 1) + '  |  ' + hrs;
          })
        : ['  _No appliances recorded_'];

      // BOM lines — item + spec + qty
      const bomLines = (r.bom || []).map(row =>
        '  • ' + (row.item || '') + (row.spec ? ' (' + row.spec + ')' : '') + '  \u00d7' + (row.qty || 1)
      );

      const lines = [
        '\u2600\ufe0f *ENERGY GUIDE \u2014 Solar System Specification*',
      ...
      .concat([
        '',
        '_Sized by Energy Guide \u2014 energyguide.app_',
        '_Contact an installer or vendor for an exact quote with pricing._',
      ]);
