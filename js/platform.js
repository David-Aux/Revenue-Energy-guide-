...
// ================================================================
    ...
    async function renderJobLeads() {
      ...
      // Timeout guard — if query takes >10s, show error instead of hanging
      let queryTimedOut = false;
      const timeoutId = setTimeout(() => {
        queryTimedOut = true;
        container.innerHTML = '<p style="color: #ef4444; text-align: center; padding: 40px;">Could not load leads — check your connection and try again.</p>';
      }, 10000);
      ...
      // Include all installer-relevant statuses so no leads are accidentally hidden
      ...
      if (queryTimedOut) return; // timeout already showed the error message

      if (error) {
        console.error('Failed to load installer leads:', error);
        container.innerHTML = '<p style="color: #ef4444; text-align: center; padding: 40px;">Failed to load job leads. Please try again.</p>';
        return;
      }
