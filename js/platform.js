// EG-VERSION: v304-fix
    ...
    async function saveCompleteInstallerProfile() {
      ...
      ['cipFullName','cipBusiness','cipPhone','cipState','cipCity','cipYears','cipMarket'].forEach(id => {
      ...
      if (!years)        { document.getElementById('cipYearsError').textContent = 'Required'; hasError = true; }
      if (!market)       { document.getElementById('cipMarketError').textContent = 'Required'; hasError = true; }
      if (hasError) return;

      showLoading(true, 'Saving profile...');
      try {
        // Upload CAC doc if provided
        ...
        const payload = {
          ...
          phone, whatsapp_number: whatsapp,
          state, city,
          service_area: service_city || city,
          service_state: service_state || state,
          min_kva: min_kva ? parseFloat(min_kva) : null,
          max_kva: max_kva ? parseFloat(max_kva) : null,
    ...
    async function saveCompleteVendorProfile() {
      ...
      // Clear previous errors
      ['cvpFullName','cvpBusiness','cvpPhone','cvpState','cvpCity','cvpDelivery'].forEach(id => {
        const err = document.getElementById(id + 'Error');
        if (err) err.textContent = '';
      });
      const prodErr = document.getElementById('cvpProductsError');
      ...
      if (!delivery)     { document.getElementById('cvpDeliveryError').textContent = 'Required'; hasError = true; }
      if (![panels, inverters, lithium, tubular, accessories].some(Boolean)) {
        if (prodErr) prodErr.textContent = 'Select at least one product category';
        hasError = true;
      }
      if (hasError) return;
      ...
      try {
        ...
        const payload = {
          ...
          brands_carried: brands || null,
          cac_number: cac_number || null,
          cac_doc_url,
          business_description: description || null,
          offers_installation: offersInstall || null,
