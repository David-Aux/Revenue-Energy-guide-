    ...
    function handleUserCalcResult(result, apps = []) {
      ...
      // ── Reset blueprint unlock for new calc ──────────────
      if (typeof egResetBlueprintForNewCalc === 'function') egResetBlueprintForNewCalc();
      ...
      // Apply paywall UI to cables section
      if (typeof egApplyBlueprintUI === 'function') egApplyBlueprintUI();
    ...
    function openUserCostScreen() {
      if (!userCalculationResult) { showToast('Calculate a system first', 'error'); return; }
      // If not unlocked, show paywall instead of full cost screen
      if (typeof egBlueprintIsUnlocked === 'function' && !egBlueprintIsUnlocked()) {
        egShowBlueprintPaywall(userCalculationResult._cloudId || null);
        return;
      }
    ...
    async function saveCurrentSystemForUser(skipPrompt = false) {
      ...
      // Must unlock blueprint before saving
      if (typeof egBlueprintIsUnlocked === 'function' && !egBlueprintIsUnlocked()) {
        egShowBlueprintPaywall(userCalculationResult._cloudId || null);
        return;
      }
    ...
    function downloadSystemPDF() {
      ...
      if (typeof egBlueprintIsUnlocked === 'function' && !egBlueprintIsUnlocked()) {
        egShowBlueprintPaywall(userCalculationResult._cloudId || null); return;
      }
    ...
    function loadUserSavedCalculation(cloudId) {
      ...
      getSavedCalculationsSafe().then(calcs => {
        ...
        // Restore unlock state from saved calculation
        if (calc.is_unlocked && typeof egGrantBlueprintAccess === 'function') {
          egGrantBlueprintAccess(calc._cloudId || null);
        }
