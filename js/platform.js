// EG-VERSION: v304-fix
    ...
    function prepareLeadFlow(target) {
      ...
      if (subtitleEl) subtitleEl.textContent = isVendor
        ? 'Tell us what you need so the vendor can prepare a quote'
        : 'Tell us a few details so we can match you properly';

      // Show/hide vendor-specific quote type field
      const vendorGroup = document.getElementById('leadVendorQuoteGroup');
      if (vendorGroup) vendorGroup.style.display = isVendor ? 'block' : 'none';

      // Show selected target summary (vendor / installer name)
    ...
    async function submitLead() {
      ...
      const budget      = document.getElementById('leadBudgetRange')?.value        || '';
      const battery     = document.getElementById('leadBatteryPreference')?.value  || '';
      const contact     = document.getElementById('leadContactMethod')?.value      || '';
      const quoteType   = document.getElementById('leadVendorQuoteType')?.value    || '';
      ...
      try {
        const lead = {
          ...
          budget_range:    budget,
          battery_pref:    battery,
          contact_method:  contact,
          note,
          system_snapshot: snapshot,
          target_type:     currentLeadTarget,
          target_id:       currentLeadTarget === 'vendor' ? selectedVendorId : selectedInstallerId,
          vendor_quote_type: currentLeadTarget === 'vendor' ? quoteType : null,
