        ...
        container.innerHTML = data.map(p => `
          <div style="display:flex;gap:10px;align-items:flex-start;
            ...
            <div style="flex:1;min-width:0;">
              <div style="font-weight:600;font-size:13px;">${safeText(p.name || 'Unnamed product')}</div>
              <div style="color:#6b7280;font-size:12px;">${safeText(typeLabels[p.category] || p.category || '')}${p.power_watts ? ' · ' + p.power_watts + 'W' : ''}${p.brand ? ' · ' + safeText(p.brand) : ''}</div>
