    ...
    async function renderSavedCalculations() {
      ...
      try {
        // Always fetch fresh from Supabase — never use stale cache here
        ...
        container.innerHTML = calcs.map(calc => {
          // Build a readable title from whichever fields are present
          const kva   = calc.invKva || calc.invKva || null;
          const kw    = calc.requiredInverter ? Math.ceil(calc.requiredInverter / 1000) : null;
          const title = kva
            ? `${kva}kVA System`
            : kw
              ? `${kw}kW System`
              : (calc.title || 'Saved System');
          const panels  = calc.numPanels  || calc.totalPanels  || '—';
          const batKwh  = calc.lithiumPackKwh || calc.batteryBankKwh || '—';
          const dateStr = calc.timestamp
            ? new Date(calc.timestamp).toLocaleDateString('en-NG', { day:'numeric', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit' })
            : '—';
          return \`
            <div class="saved-calc-item">
              <div class="saved-calc-title">\${title}</div>
              <div style="font-size:12px;color:#6b7280;margin:2px 0 8px;">
                \${panels} panels · \${batKwh}kWh battery
              </div>
        ...
        container.innerHTML = '<p style="color:#ef4444; text-align:center; padding:40px;">Failed to load calculations. Check your connection.</p>';
      }
    }
    ...
    async function deleteCalc(cloudId) {
      ...
      try {
        ...
        // Force clear cache so renderSavedCalculations fetches fresh from Supabase
