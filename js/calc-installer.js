  ...
  function l4i__buildBOM(s,bi){
    ...
    s.bom=bom;
    s.budget=l4i__calcBudgetRange(s,bi);
    s.total=s.budget.min; // keep .total for legacy references, points to low end
    return s;
  }
  ...
  function l4i__renderResults(s){
    ...
    // Bridge to parent
    var out={totalWatts:s.tw,dailyKwh:s.dw/1000,maxSurge:Math.round(s.surge),invKva:s.inv.kva,sysV:s.V,systemVoltage:s.V,lithiumPackKwh:b0.totalKwh||0,batUnits:b0.units||1,batLabel:b0.label||'',numPanels:s.panels,panelCount:s.panels,pvWatts:s.arrW,requiredPvWatts:s.reqWActive||s.reqW,panelMode:s.activePanelMode||'A',pvCable:s.conn.pv.cab.s,battCable:s.conn.bat.cab.s,acCable:s.conn.ac.cab.s,pvBreaker:s.conn.pv.brk.a,battBreaker:s.conn.bat.brk.a,acBreaker:s.conn.ac.brk.a,budgetMin:s.budget?s.budget.min:0,budgetMax:s.budget?s.budget.max:0,totalCost:s.budget?s.budget.min:0,bom:s.bom||[],tubularUnitPrice:0,tubularAltCost:0,leadTotalBatteries:0};
  ...
  function l4i__renderBOM(s){
    var body=document.getElementById('l4i-r-bom-body'),tot=document.getElementById('l4i-r-bom-total'),range=document.getElementById('l4i-r-budget-range');
    if(!body)return;
    ...
    if(range&&s.budget){
      var fmtM=function(n){return '₦'+Number(Math.round(n/1000)).toLocaleString('en-NG')+'k';};
      range.innerHTML='<div style="background:linear-gradient(135deg,rgba(34,197,94,0.12),rgba(245,158,11,0.10));border:1px solid var(--border,#334155);border-radius:12px;padding:14px 16px;margin-top:4px;">'+
        '<div style="font-size:10px;text-transform:uppercase;letter-spacing:.8px;color:var(--muted);font-weight:700;margin-bottom:6px;">Estimated System Cost</div>'+
        '<div style="font-size:22px;font-weight:800;color:var(--sun);">'+fmtM(s.budget.min)+' – '+fmtM(s.budget.max)+'</div>'+
        '<div style="font-size:11px;color:var(--muted);margin-top:6px;line-height:1.5;">Range reflects market price variation across brands and suppliers. Your installer will provide an exact quote.</div>'+
      '</div>';
    }
  }
